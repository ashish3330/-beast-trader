...
"""5-fold STRICT walk-forward validation for UK100.r / momentum.

Winner config (FROZEN — no per-fold re-tuning):
  SL_ATR_MULT      = 2
  SIGNAL_QUALITY   = {trending:50, ranging:55, volatile:50, low_vol:85}
  PULLBACK_RETRACE = 0
  PULLBACK_WAIT    = 1
...
SYMBOL = "UK100.r"
OUT_DIR = ROOT / "backtest" / "results" / "wf_strict5_20260617"
...
def run_fold(fold):
    ...
    p["pullback_retrace"] = float(WINNER_PULLBACK_RETRACE)
    p["pullback_atr_retrace"] = float(WINNER_PULLBACK_RETRACE)
    p["pullback_wait"] = int(WINNER_PULLBACK_WAIT)
    p["pullback_max_wait"] = int(WINNER_PULLBACK_WAIT)
...
def main():
    ...
    print(f"Winner: SL={WINNER_SL_ATR_MULT}  Q={WINNER_SIGNAL_QUALITY}  "
          f"PB_R={WINNER_PULLBACK_RETRACE}  PB_W={WINNER_PULLBACK_WAIT}  "
