...
def main() -> None:
    ...
    # NOTE 2026-05-09: previous component-weights tune produced overfit signature
    # (PF 25-40 with n=23-100 trades). Live loader was disabled to prevent re-seed.
    # If you intend to restore: rename the disable marker, validate walk-forward, then
    # rename this output to component_weights_auto_dict.py manually. Refusing here so
    # an accidental run cannot reactivate the overfit profile.
    disable_marker = RESULTS / "component_weights_auto_dict.py.disabled"
    if disable_marker.exists():
        print("\n⚠️  REFUSING to write component_weights_auto_dict.py — overfit-disabled marker present.")
        print(f"   Marker: {disable_marker}")
        print( "   To re-enable: review walk-forward results, then `mv` the marker out of the way.")
        return
