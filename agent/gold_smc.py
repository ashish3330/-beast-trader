...
Signal (LONG mirror SHORT):
  bias bullish (D1 close > D1 EMA50) + recent liquidity sweep (took a low, reclaimed)
  within SEQ bars + BOS (close > prior swing high) + bullish FVG + EMA9>EMA21 both
  up + close>VWAP + (MACD>signal OR RSI>50) + strong/engulfing candle.
...
def evaluate(h1, d1_bias, params=None):
    ...
    long_ok = (d1_bias == 1 and lbs is not None
               and g(c, T) > g(prior_hi, T) and bull_fvg
               and g(ema9, T) > g(ema21, T) and g(ema9, T) > g(ema9, T-1) and g(ema21, T) > g(ema21, T-1)
               and g(c, T) > g(vwap, T) and (g(macd, T) > g(sigl, T) or g(rsi, T) > 50)
               and (g(body, T) > 1.2*g(avgbody, T) or be))
    short_ok = (d1_bias == -1 and lbsh is not None
                and g(c, T) < g(prior_lo, T) and bear_fvg
                and g(ema9, T) < g(ema21, T) and g(ema9, T) < g(ema9, T-1) and g(ema21, T) < g(ema21, T-1)
                and g(c, T) < g(vwap, T) and (g(macd, T) < g(sigl, T) or g(rsi, T) < 50)
                and (g(body, T) > 1.2*g(avgbody, T) or se))
    entry = g(c, T)
    if long_ok:
