...
"""
SMA Breakout (SMABO) backtester — validation gate before flipping
SMABO_TRADE_LIVE = True.

Mirrors backtest/sweep_reclaim_backtest.py exactly:
  * Imports SMABreakoutStrategy from agent/sma_breakout.py AS-IS (no
    detector re-implementation).
  ...
  * Simulates each fired signal through TP1 / TP2 / SL / trail.

CLI:
    python3 -B backtest/sma_breakout_backtest.py
    python3 -B backtest/sma_breakout_backtest.py --symbol XAUUSD --days 180

Output: per-symbol PF / WR / total R / max DD, and an aggregate row.
"""
import argparse
import pickle
import sys
...
# Per-asset spread in price units — mirrors sweep_reclaim_backtest.py.
SPREAD = {
...
# Default universe — narrow start aligned with config.SMABO_WHITELIST.
UNIVERSE = ["XAUUSD", "EURUSD", "BTCUSD", "DJ30.r", "SP500.r"]
START_CAPITAL = 5000.0
RISK_PCT = 0.0025                # 0.25%/trade — matches config.SMABO_RISK_PCT
MIN_BARS_FOR_BT = 2000
M15_WINDOW = 800                 # 800 M15 bars ≈ 8 trading days ≈ 200 H1
H1_WINDOW = 240
...
def _resample_h1(m15):
    """M15 → H1 (the strategy then resamples H1 → H4 internally)."""
...
class _FakeState:
    """Truncate the cache at the cursor and serve trailing windows with a
    synthetic forming bar appended (so the strategy's iloc[-2] resolves to
    the cursor bar, mimicking live SharedState shape)."""
...
def _simulate_trade(sig, m15_arr, entry_i, spread):
    ...
    Mirror of sweep_reclaim_backtest._simulate_trade with SMABO twist:
    runner uses SMA20-based trail (replicated cheaply via 0.5×ATR step
    on the C array; the live SMA20 trail is enforced by executor at
    runtime, which is structurally identical for the BT outcome class).
    """
    H, L, C = m15_arr
    n = len(C)
...
def backtest_symbol(symbol, days=None):
    ...
    trades = []
    open_until = -1
    # Need enough warm-up: at least 800 M15 bars for the H4 window to be full.
    start_i = max(800, 60)
...
def main():
    parser = argparse.ArgumentParser(description="SMABO Backtest")
    parser.add_argument("--days", type=int, default=None,
                        help="Trailing window in days (default: full cache).")
    parser.add_argument("--symbol", type=str, default=None,
                        help="Single symbol (default: SMABO_WHITELIST).")
    ...
    print("\n" + "=" * 120)
    print("  SMABO BACKTEST  —  validation gate before SMABO_TRADE_LIVE=True")
    print("=" * 120)
